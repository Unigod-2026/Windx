...
def questions_analytics(
    ...
    """Aggregate KPIs for the 问题提及分析 tab, server-side.

    The frontend used to pull a paginated detail list
    (``/brand-mentions``) and roll up the per-prompt numbers in a
    ``useMemo``. That drifted because the detail endpoint caps ``size``
    at 100 — a 15-day window over a busy project easily produces
    ...
    deliberately keyed by prompt text so adding / removing prompts
    in the editor flows through here without an extra mapping step.
    """
    project = _get_project(db, project_id)
    _assert_customer_access(user, project)
    ...
    # Prompts with zero brand_mention rows in the window still show up
    # (so the UI can render "no data" placeholders) — but only those
    # that exist in geo_project_prompts; rows from old / deleted
    # prompts are dropped.
    ...
    prompt_meta = {
        prompt: {"id": pid, "category": cat, "status": status_}
    ...
    def _window_stats(start_dt: datetime, end_dt: datetime) -> dict[str, dict]:
        ...
        rows = db.execute(
            select(
                BrandMention.prompt,
                BrandMention.platform,
                # total runs for (prompt, platform)
        ...
        for prompt, platform, total, matched, best_rank, avg_sent, recommend_int in rows:
            meta = by_prompt.setdefault(
                prompt or "(空问题)",
                {
                    "total": 0,
                    "matched": 0,
            ...
            meta["matched"] += int(matched or 0)
            meta["coverage"] += 1
            # Per-platform stats stay here; per-prompt rank aggregates
            # are computed in a second pass below.
        ...
        # sum of rank==1, sum of rank values across all rows that have a
        # rank). Doing it as a separate query keeps the platform-level
        # GROUP BY above cheap.
        rank_rows = db.execute(
            select(
                ...
                func.avg(BrandMention.rank_position).label("rank_avg"),
            )
            .where(
                BrandMention.project_id == project_id,
                BrandMention.is_self.is_(is_self_filter),
        ...
        for prompt, top1, top3, rank_avg in rank_rows:
            meta = by_prompt.get(prompt or "(空问题)")
            if meta is None:
                # All rows for this prompt have rank_position=null — no
                # (prompt, platform) rows matched at all, so it wasn't
                # added by the first pass. Synthesize an empty entry
                # so the prompt still appears with its KPIs zeroed out.
                meta = {
                    "total": 0,
                    "matched": 0,
                    "top1": 0,
    ...
    for prompt, meta in prompt_meta.items():
        ...
        items.append(
            QuestionAnalyticsItem(
                ...
                mention_rate=(matched / n) if n else 0.0,
                rank_avg=cur.get("rank_avg") if cur else None,
                coverage=cur["coverage"] if cur else 0,
                platforms=[
                    QuestionPlatformStat(
                        platform=plat,
                        matched=p["matched"],
                ...
                prev=QuestionPrevStat(
                    total=prev["total"],
                    matched=prev["matched"],
                    top1_rate=(prev["top1"] / prev["total"]) if prev["total"] else 0.0,
                    top3_rate=(prev["top3"] / prev["total"]) if prev["total"] else 0.0,
                    mention_rate=(prev["matched"] / prev["total"]) if prev["total"] else 0.0,
                    rank_avg=prev.get("rank_avg"),
                )
