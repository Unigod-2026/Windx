 ...
 *
 * 每个 point 是 (platform_code, delivery_mode, thinking_mode) triple,
 * ``platformLabel`` 把 compound key 渲染成「千问-网页-快速」格式。
 */
export default function QuadrantChart({ points, selfAvg, competitorAvg }: Props) {
  const w = 1400, h = 520;
  const padL = 110, padR = 48, padT = 36, padB = 60;
        ...
        return (
          <g key={p.platform}>
            <circle cx={x(p.self_mention_rate)} cy={y(p.competitor_avg_mention_rate)} r={10} fill="#1a55e8">
              <title>{`${label}: 自身 ${(p.self_mention_rate * 100).toFixed(0)}% · 竞品均值 ${(p.competitor_avg_mention_rate * 100).toFixed(0)}%`}</title>
            </circle>
